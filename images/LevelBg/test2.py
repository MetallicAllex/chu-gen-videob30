...
def FrameLoader(level_index: int = 0):
    with Image.open(f"./static/assets/images/Chunithm/Frames/{level_index}.png") as _frame:
        return _frame.copy()

def LevelLoader(ds_cur: float, ds_next: float = 0.0):
    ...
    level_number_img = TextDraw(level_number_img, level, (54, 46), 
                                     font_path="./static/assets/fonts/NimbusSanL-Bol.otf",
...
def ScoreLoader(score: int = 0):
    ...
    for char in score_str_formatted:
        if char == ',':
            image_path = f"./static/assets/images/Chunithm/Numbers/AchievementNumber/comma.png"
            char_width = comma_size[0] // 2
        else:
            image_path = f"./static/assets/images/Chunithm/Numbers/AchievementNumber/{char}.png"
...
def RatingLoader(rating: float):
    ...
    for char in ra_number_formatted:
        if char == '.':
            image_path = f"./static/assets/images/Chunithm/Numbers/RatingNumber/{digit_style}/dot.png"
            char_width = dot_size[0] // 2
            char_y = 8  # 小数点位置靠下
        else:   
            image_path = f"./static/assets/images/Chunithm/Numbers/RatingNumber/{digit_style}/{char}.png"
            char_width = digit_size[0]
            char_y = 0
        with Image.open(image_path) as char_img:
            if char == '.':
...
def ComboStatusLoader(combo_status: str = ""):
    if combo_status == '' or combo_status is None:
        return Image.new('RGBA', (80, 80), (0, 0, 0, 0))
    elif combo_status == 'fc':
        with Image.open(f"./static/assets/images/Chunithm/ComboStatus/11.png") as _comboStatus:
            return _comboStatus.copy()
    elif combo_status == 'aj':
        with Image.open(f"./static/assets/images/Chunithm/ComboStatus/12.png") as _comboStatus:
            return _comboStatus.copy()
    elif combo_status == 'ajc':
        with Image.open(f"./static/assets/images/Chunithm/ComboStatus/13.png") as _comboStatus:
            return _comboStatus.copy()
    else:
        return Image.new('RGBA', (80, 80), (0, 0, 0, 0))

def ChainStatusLoader(chain_status: str = ""):
    if chain_status == '' or chain_status is None:
        return Image.new('RGBA', (80, 80), (0, 0, 0, 0))
    elif chain_status == 'fc':
        with Image.open(f"./static/assets/images/Chunithm/ComboStatus/21.png") as _chainStatus:
            return _chainStatus.copy()
    elif chain_status == 'fcr':
        with Image.open(f"./static/assets/images/Chunithm/ComboStatus/22.png") as _chainStatus:
            return _chainStatus.copy()
    else:
        return Image.new('RGBA', (80, 80), (0, 0, 0, 0))
...
def TextDraw(image, text: str = "", pos: tuple = (0, 0), max_width: int = 2000,
    ...
    if not font_path:
        font_path = "./static/assets/fonts/SOURCEHANSANSSC-BOLD.OTF"
...
def GenerateOneAchievement(record_detail: dict):
    """
    生成单个Chunithm成绩记录。
    ...
    try:
        ...
        image_base_path = os.path.join(os.getcwd(),
                                        f"./static/assets/images/Chunithm/content_base_chunithm_verse.png")
        with Image.open(image_base_path) as background:
            ...
            # 加载等级
            level_pos = (102, 884)
            level = LevelLoader(record_detail["ds_cur"], record_detail["ds_next"])
            ...
            if ds_cur <= 0.0:  # 不在当前版本的谱面，使用0来标记无定数
                ds_cur_text = "--"
            _temp_img = TextDraw(_temp_img, ds_cur_text , ds_cur_pos,
                                      font_path="./static/assets/fonts/SweiBellLegCJKsc-Black.ttf", 
            ...
            if ds_cur <= 0.0: 
                ...
                ds_next_text = str(ds_next) + "→"
            _temp_img = TextDraw(_temp_img, ds_next_text , ds_next_pos,
                                      font_path="./static/assets/fonts/SweiBellLegCJKsc-Black.ttf", 
                                      font_size=45, font_color=(77, 77, 77), h_align="center")
            ...
            # 加载Rating值
            rating_pos = (1216, 980)
            rating = RatingLoader(record_detail['ra'])
            _temp_img.paste(rating, rating_pos, rating)

            # 加载Combo状态
            combo_status_pos = (426, 975)
            combo_status = ComboStatusLoader(record_detail["combo_type"])
            ...
            chain_status = ChainStatusLoader(record_detail["chain_type"])
            ...
            _temp_img = TextDraw(_temp_img, title, text_title_pos, max_width=900,
                                      font_path="./static/assets/fonts/SweiBellLegCJKsc-Black.ttf", 
                                      font_size=48, font_color=(26, 0, 84), h_align="left")
            # 艺术家
            text_artist_pos = (234, 936)
            artist = record_detail['artist']
            _temp_img = TextDraw(_temp_img, artist, text_artist_pos, max_width=420,
                                      font_path="./static/assets/fonts/SweiBellLegCJKsc-Black.ttf", 
            ...
            #     play_count = 0
            # if play_count >= 1:
            #     with Image.open(f"./static/assets/images/Chunithm/Playcount/PlayCountBase.png") as PlayCountBase:
            #         _temp_img.paste(PlayCountBase, (1177, 846), PlayCountBase)
            #     text_center_pos = (1359, 865)
            #     _temp_img = TextDraw(_temp_img, str(play_count), text_center_pos,
            #                               font_path="./static/assets/fonts/SweiBellLegCJKsc-Black.ttf",
...
if __name__ == "__main__":
    ...
    # 生成成绩图片
    result = GenerateOneAchievement(record_detail)
    result.save("chunithm_achievement.png")