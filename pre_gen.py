...
def update_b50_data_lxns(b50_raw_file, b50_data_file, friend_code):
    ...
    return _process_b50_data(lxns, "lxns", b50_raw_file, b50_data_file)

def update_b50_data_fish(b50_raw_file, b50_data_file, username):
    try:
        fish = get_b50_data_from_fish(username)
        if 'message' in fish:
            raise ConnectionError(f"请求 Best50 数据失败: {fish['message']}")
        return _process_b50_data(fish, "fish", b50_raw_file, b50_data_file)
...
# def download_b30_videos(downloader, b50_data, video_download_path, download_wait_time=(0,0)):
...
# def st_gen_resource_config(b50_data, 
