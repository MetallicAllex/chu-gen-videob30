        ...
        with st.expander("前景素材", icon="🖼️"):
                ...
                with tab_bg:
                    if sel_preset_style == "default" and themes == "default":
                        ...
                        st.error("init 样式的谱面确认框架与难度框合并，无法在此处修改。", icon="❌")
                ...
                with tab_frames:
                    ...
                    if st.session_state[frame_state_key]:
                        ...
                        with frame_col1:
                            st.success(f"{frame_levels[selected_level]} 边框已更新为：{st.session_state[frame_state_key]['name']}", icon="✅")
                        with frame_col2:
...
# 在文件开头添加通用配置渲染函数
def render_simple_config(data, config_type, display_names, columns_per_row=4, sel_preset_style=None):
    """
    渲染配置项（支持颜色、大小、宽度、对齐等）
    
    Args:
        data: 配置数据字典（支持嵌套结构）
        config_type: 配置类型 ('color', 'size', 'maxWidth', 'align')
        display_names: 键名到显示名称的映射字典
        columns_per_row: 每行显示的列数
        sel_preset_style: 当前选中的样式（用于判断是否可编辑）
    """
    # 配置类型信息
    config_info = {
        'color': {
    ...
    is_color_config = config_type == 'color'
    
    # 收集所有配置项（支持嵌套结构）
    def collect_items(data, prefix="", items=None):
        """递归收集配置项"""
        if items is None:
            items = []
        
        for key, value in data.items():
            if isinstance(value, dict) and is_color_config:
                # 只有颜色配置需要递归处理嵌套字典
                collect_items(value, f"{prefix}{key}.", items)
            elif not isinstance(value, dict):  # 非字典类型作为配置项
                display_name = display_names.get(key, key)
                items.append({
                    'path': prefix + key,
                    'display': display_name,
                    'value': value,
                    'key_parts': key  # 用于非嵌套结构
                })
            elif not is_color_config:
                # 非颜色配置但遇到字典（不应该发生，但做个保护）
                collect_items(value, f"{prefix}{key}.", items)
        
        return items
    
    # 收集配置项
    config_items = collect_items(data)
    ...
    with st.expander(f"{info['name']}"):
        ...
        # 计算行数和列布局
        rows = (len(config_items) + columns_per_row - 1) // columns_per_row
        
        for row_idx in range(rows):
            cols = st.columns(columns_per_row)
            start_idx = row_idx * columns_per_row
            end_idx = min(start_idx + columns_per_row, len(config_items))
            
            for col_idx, item in enumerate(config_items[start_idx:end_idx]):
                with cols[col_idx]:
                    # 根据配置类型渲染不同的控件
                    if info['widget'] == 'color_picker':
                        # 颜色选择器
                        hex_color = f"#{item['value'][0]:02x}{item['value'][1]:02x}{item['value'][2]:02x}"
                        new_value_hex = st.color_picker(
                            item['display'],
                            hex_color,
                        ...
                        if new_value_hex:
                            # 转换回RGB
                            new_value = [
                                int(new_value_hex[1:3], 16),
                                int(new_value_hex[3:5], 16),
                                int(new_value_hex[5:7], 16)
                            ]
                            ...
                            if '.' in item['path']:
                                ...
                                data[item['path']] = new_value
                    
                    elif info['widget'] == 'number_input':
                        # 数字输入框
                        new_value = st.number_input(
                            item['display'],
                            value=item['value'],
                            key=f"{config_type}_{item['key_parts']}",
                            disabled=sel_preset_style != "自定义",
                            help=info['help'],
                            **info['params']
                        )
                        # 更新数据
                        data[item['key_parts']] = new_value
                    
                    elif info['widget'] == 'selectbox':
                        # 下拉选择框
                        options = info['params']['options']
                        current_index = options.index(item['value']) if item['value'] in options else 0
...
if os.path.exists(current_paths['custom_style']):
    ...
    with st.expander("画面参数", icon="📝"):
        ...
        image_data = custom_data["position"]["image"]
        # 设置 use_expander=False 让 image 本身不使用 expander
        with st.expander("坐标设置"):
            render_coordinates("image", custom_data["position"]["image"], name_mapping=image_name_mapping, use_expander=False)
