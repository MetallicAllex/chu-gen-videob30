...
class TextRenderer:
    ...
    def render(self, text: str, auto_scale: bool = True) -> Image.Image:
            ...
            for line in lines:
                if line:
                    with Pilmoji(temp_img, source=_get_bootcdn_source()()) as pilmoji:
                        ...
                        # 修复：检测是否包含 emoji
                        has_emoji_in_line = self._has_emoji(line)
                        if has_emoji_in_line:
                            # Emoji 需要更多空间，使用字体大小的 1.5 倍
                            line_height = int(self.style.font_size * 1.5)
                        else:
                            # 普通文本使用实际高度和基础行高的最大值
                            line_height = max(h, base_line_height)
        ...
        if self.layout.auto_height:
            # 添加安全边距防止 emoji 被截断
            safe_margin = self.style.font_size // 2 if has_emoji else 0
            image_height = total_text_height + self.layout.padding[0] + self.layout.padding[2] + safe_margin
        else:
            image_height = self.layout.height
        ...
        if self.layout.vertical_align == "top":
            ...
            current_y = self.layout.padding[0] + (image_height - self.layout.padding[0] - self.layout.padding[2] - content_height) // 2
        else:
            current_y = image_height - self.layout.padding[2] - total_text_height
